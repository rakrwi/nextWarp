...
import com.heulim.nextwarp.api.NextWarpAPI
...
import com.heulim.nextwarp.common.data.WarpLocation
import com.heulim.nextwarp.common.database.DatabaseManager
import com.heulim.nextwarp.common.redis.RedisManager
import com.heulim.nextwarp.paper.command.*
import com.heulim.nextwarp.paper.listener.PlayerListener
...
class NextWarpPaper : JavaPlugin() {
    ...
    private fun handleTeleportRequest(request: TeleportRequest) {
        ...
        // 워프 설정 요청 처리
        if (request.warpName != null && request.warpName.startsWith("SET:")) {
        ...
        when (request.type) {
            TeleportType.WARP -> {
                if (request.world != null && request.x != null && request.y != null && request.z != null) {
                    val world = Bukkit.getWorld(request.world)
                    if (world != null) {
    ...
    private fun handleRandomTp(player: org.bukkit.entity.Player, request: TeleportRequest) {
        val world = if (request.world != null) {
            Bukkit.getWorld(request.world)
        } else {
