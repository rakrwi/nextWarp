package dev.rakrwi.nextwarp.paper.listener
...
class PlayerListener(private val plugin: NextWarpPaper) : Listener {
    ...
    fun onPlayerSpawnLocation(event: PlayerSpawnLocationEvent) {
        ...
        when (pendingRequest.type) {
            TeleportType.WARP -> {
                val worldName = pendingRequest.world
                val x = pendingRequest.x
                val y = pendingRequest.y
                val z = pendingRequest.z
                if (worldName != null && x != null && y != null && z != null) {
                    val world = Bukkit.getWorld(worldName)
                    if (world != null) {
                        val location = Location(world, x, y, z, pendingRequest.yaw ?: 0f, pendingRequest.pitch ?: 0f)
            ...
            TeleportType.TPA -> {
                ...
                if (worldName != null && x != null && y != null && z != null) {
                    ...
                    if (world != null) {
                        ...
                        plugin.logger.warning("[TPA-DEBUG] 월드를 찾을 수 없음: $worldName")
                    }
                } else if (!pendingRequest.tpaType.isNullOrEmpty()) {
                    // tpaType이 있는 경우 (하위 호환성)
                    val targetPlayerName = pendingRequest.targetPlayerName
                    when (pendingRequest.tpaType) {
                        "do_teleport" -> {
                            plugin.logger.info("[TPA-DEBUG] do_teleport 스폰 처리 (deprecated): player=${player.name}, targetPlayer=$targetPlayerName")
                            if (targetPlayerName != null) {
                                val targetPlayer = getPlayerIgnoreCase(targetPlayerName)
                        ...
                        "do_teleport_coords" -> {
                            ...
                            val w = pendingRequest.world
                            val px = pendingRequest.x
                            val py = pendingRequest.y
                            val pz = pendingRequest.z
                            if (w != null && px != null && py != null && pz != null) {
                                val world = Bukkit.getWorld(w)
                                if (world != null) {
                                    val location = Location(world, px, py, pz, pendingRequest.yaw ?: 0f, pendingRequest.pitch ?: 0f)
                                    event.spawnLocation = location
                                    plugin.pendingTeleports[player.uniqueId] = pendingRequest.copy(tpaType = "do_teleport_done")
                                }
                            }
